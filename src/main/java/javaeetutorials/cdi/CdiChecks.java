...
public class CdiChecks {
    ...
    public Response ping() {
        return Response.ok().entity(
                greeting + new SimpleDateFormat(customFormat).format(new Date()) + " log: " + log.printPlace()
        ).build();
    }
    ...
    @GET
    @Path("/add")
    public Response addLaptop(@QueryParam("name") String name, @QueryParam("version") String version) {
        return Response.ok().entity(laptopService.create(name, version).toString()).build();
    }

    @GET
    @Path("/remove")
    public Response removeLaptop(@QueryParam("name") String name, @QueryParam("version") String version) {
